...
class Solution   {
	...
	void reinit() {
		valeursLibres.clear();
		for (int nb : nombres) valeursLibres.add(nb);
		etapes.delete(0, etapes.length());
	}

	/**
	 * fonction de calcul d'une solution par méthode aléatoire
	...
	public int calcule() {
		...
		//tant que le dernier resultat est différent du but et qu'il reste au moins 2 nombres à choisir
		while ((dernierCalcul != but)  && (nbValLibres > 1)) {
			...
			if ( (Math.abs(dernierCalcul - but) < Math.abs(bestSolution - but))) {
	...
	OperateurFunc choixOperateurApplicable(int nb1, int nb2) {
		...
		boolean ok = operateurs.get(i).test.apply(nb1, nb2);
		while (!ok) {
		   i = (i+1)%4;
...
//		System.out.println(Thread.currentThread().getName()+ ",nb cycles = " +nbCalculs );
	...
	public static void main(String[] args) {
		...
		while (groupe.activeCount()==5) Thread.yield();
		groupe.interrupt();
